/*
 * WDK v2 Native Engine — SHA-512 Implementation (FIPS 180-4)
 */
...
void wdk_sha512_init(wdk_sha512_ctx *ctx)
{
    ...
    ctx->count_lo = 0;
    ctx->count_hi = 0;
    memset(ctx->buffer, 0, WDK_SHA512_BLOCK_SIZE);
}

void wdk_sha512_update(wdk_sha512_ctx *ctx, const uint8_t *data, size_t len)
{
    size_t buffered = (size_t)(ctx->count_lo % WDK_SHA512_BLOCK_SIZE);
    ...
    if (buffered > 0) {
        size_t need = WDK_SHA512_BLOCK_SIZE - buffered;
        if (len < need) {
    ...
    while (len >= WDK_SHA512_BLOCK_SIZE) {
        sha512_transform(ctx->state, data);
        data += WDK_SHA512_BLOCK_SIZE;
        len  -= WDK_SHA512_BLOCK_SIZE;
    }
...
{
    /* Total bits = count * 8, stored as 128-bit big-endian */
    ...
    if (buffered > 112) {
        memset(ctx->buffer + buffered, 0, WDK_SHA512_BLOCK_SIZE - buffered);
        sha512_transform(ctx->state, ctx->buffer);
        buffered = 0;
    }

    memset(ctx->buffer + buffered, 0, 112 - buffered);
...
void wdk_sha512(const uint8_t *data, size_t len, uint8_t out[64])
{
    wdk_sha512_ctx ctx;
    wdk_sha512_init(&ctx);
    wdk_sha512_update(&ctx, data, len);
