/*
 * WDK v2 Native Engine — SHA-256 Implementation (FIPS 180-4)
 */
...
void wdk_sha256_init(wdk_sha256_ctx *ctx)
{
    ...
    ctx->count = 0;
    memset(ctx->buffer, 0, WDK_SHA256_BLOCK_SIZE);
}

void wdk_sha256_update(wdk_sha256_ctx *ctx, const uint8_t *data, size_t len)
{
    size_t buffered = ctx->count % WDK_SHA256_BLOCK_SIZE;
    ctx->count += len;

    /* If we have buffered data, try to fill the block */
    if (buffered > 0) {
        size_t need = WDK_SHA256_BLOCK_SIZE - buffered;
        if (len < need) {
    ...
    while (len >= WDK_SHA256_BLOCK_SIZE) {
        sha256_transform(ctx->state, data);
        data += WDK_SHA256_BLOCK_SIZE;
        len  -= WDK_SHA256_BLOCK_SIZE;
    }
...
void wdk_sha256_final(wdk_sha256_ctx *ctx, uint8_t out[32])
{
    ...
    if (buffered > 56) {
        memset(ctx->buffer + buffered, 0, WDK_SHA256_BLOCK_SIZE - buffered);
        sha256_transform(ctx->state, ctx->buffer);
        buffered = 0;
    }
...
void wdk_sha256(const uint8_t *data, size_t len, uint8_t out[32])
{
    wdk_sha256_ctx ctx;
    wdk_sha256_init(&ctx);
    wdk_sha256_update(&ctx, data, len);
